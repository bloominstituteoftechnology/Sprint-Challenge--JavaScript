...
1. Describe the biggest difference between `.forEach` & `.map`.
   ...
   - `.forEach()` doesnt return anything. It just allows you to call a method on each element of the array.

   - `.map()` will also let you call a function on each element in the array, the difference is that `.map()` utilizes return values and actually returns a new Array of the same size.
...
3. What is closure?
   ...
   - A close is what happens to end a function. Example.. `return item` ... `console.log("success"` ... Etc..
...
5) Why do we need super() in an extended class?
   ...
   - Because it allows you to use the attributes from the `Parent` class in your child class object.
