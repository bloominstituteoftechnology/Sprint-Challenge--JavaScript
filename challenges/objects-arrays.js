...
zooAnimals.forEach(val => animalNames.push(`${val.animal_name} ${val.scientific_name}`));
...
zooAnimals.map(val => lowerCase.push(val.animal_name.toLowerCase()));
...
largerPopulation = zooAnimals.filter(val => {return val.population < 5});
// largerPopulation = zooAnimals.filter(value => {return value.population < 5});
...
*/
const populationTotal = zooAnimals.reduce((reducer, item) => {
  return (reducer += item.population);
}, 0);
