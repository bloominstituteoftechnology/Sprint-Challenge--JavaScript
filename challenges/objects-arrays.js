...
const contactInfo = [];
for (let i = 0; i < graduates.length; i++) {
  let item = `${graduates[i].first_name} ${graduates[i].email}`
...
const uni = [];
for (let i = 0; i < graduates.length; i++) {
  let lowercase = graduates[i].university.toLowerCase();
  if(lowercase.includes('uni')){
    uni.push(lowercase);
  }
}
console.log(uni);
...
let populationTotal;
populationTotal = zooAnimals.reduce((acc, curr)=>{ return acc + curr.population; }, 0);
