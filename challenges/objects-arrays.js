...
const uni = [];
for (let i = 0; i < graduates.length; i++) {
    if (graduates[i].university.includes('Uni') || graduates[i].university.includes('uni')) {
...
const populations = zooAnimals.map(arr => arr.population)
const populationTotal = populations.reduce((acc, pop) => acc + pop);
