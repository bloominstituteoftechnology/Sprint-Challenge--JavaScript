...
const veloc = {name: "velociraptor", diet: "carnivorous", weight: "15kg", length: "1.8m", period: "Late Cretaceous"};
...
graduates.sort((a, b) => (a.university < b.university ? -1 : 1)).forEach((graduate) => universities.push(graduate.university));
...
const unisWithUni = [];
graduates.forEach((graduate) => unisWithUni.push(
  graduate.university.includes('Uni') ? graduate : null
))
...
The zoos need to know their total animal population across the United States. Find the total population from all the zoos using
 the .reduce() method. Remember the reduce method takes two arguments: a callback (which itself takes two args), 
