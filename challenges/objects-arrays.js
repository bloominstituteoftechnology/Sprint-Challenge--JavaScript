...
// stegosaurus, herbivorous, herbivorous, 9m, Late Jurassic
...
const unisWithUni = [];
for (let x = 0; x < universities.length; x++) {
  if (universities[x].includes("Uni")) {
    unisWithUni.push(universities[x]);
  }
}
...
*/
const displayNames = [];
for (let x = 0; x < zooAnimals.length; x++) {
  displayNames.push(`Name: ${zooAnimals[x].animal_name}, Scientific Name: ${zooAnimals[x].scientific_name}`);
}
