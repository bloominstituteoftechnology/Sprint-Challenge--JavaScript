...
zooAnimals.forEach(function(array) {
  animalNames.push(`${array.animal_name}, ${array.scientific_name}`)
})
...
lowerCase = animalNames.map(function(lower) {
  return lower.toLowerCase(`${lower.animal_name}`)
});
...
largerPopulation = zooAnimals.filter(zooAnimals => zooAnimals.population < 5);
...
populationTotal = zooAnimals.reduce(function(acc, zooAnimals) {
