...
//var myArr = dinosaurs.map(a => a.weight);
...
universities.sort(function(a, b) {
  const x = a.university.toLowerCase();
  const y = b.university.toLocaleLowerCase();
  if (x < y) {
    return -1;
  }
  if (x > y) {
...
let arr = zooAnimals.filter(a => a.population < 5);
lowPopulationAnimals.push(arr);
