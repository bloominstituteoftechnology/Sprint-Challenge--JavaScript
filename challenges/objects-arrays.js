...
const universities = [];
let unsort = [];
unsort = graduates.map(element => element.university);
universities.push(unsort.sort());
...
const contactInfo = [];
contactInfo.push(graduates.map(element => element.first_name + ' ' + element.email))
...
const lowerCase = [];
lowerCase.push(zooAnimals.map(a => a.animal_name.toLowerCase()));
console.log(lowerCase); 

// The zoos need to know their total animal population across the United States.  Add up all the population numbers from all the zoos using the .reduce() method.
const populationTotal = [];
populationTotal.push(zooAnimals.reduce((a, b) => a += b.population, 0));
