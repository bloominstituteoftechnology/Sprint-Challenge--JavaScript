...
zooAnimals.forEach(item =>
  animalNames.push(`animal:${item.animal_name}, scientific_name:${item.scientific_name}`)
);
...
zooAnimals.map(animal =>
  lowerCase.push(`name: ${animal.animal_name.toLowerCase()}`)
);
...
*/
const largerPopulation = zooAnimals.filter(lowPopulation =>
  lowPopulation.population < 5
  );
...
*/
const populationTotal = zooAnimals.reduce((reducer, totalPopulation) => {
  return reducer + totalPopulation.population
}, 0);
