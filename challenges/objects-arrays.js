...
const uni = graduates.filter(grad => {
  if(/Uni/.test(grad.university)) { // returns array of grad. objects with 'Uni' in their university's name
    return grad.university;
  }
}).map(grad => grad.university); // returns array of universities only
console.log(uni);
