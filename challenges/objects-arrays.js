...
/* Request 1: Create a new array called universities that contains all the univeristies in the graduates array. */
 ...
 for(let i=0; i < graduates.length; i++) {
   universities.push(graduates[i][`university`])  // adds elements to array
 }
...
zooAnimals.map(function(animalName) {
  return lowerCase.push(animalName.animal_name.toLowerCase());
});
...
const reducePopulation = zooAnimals.reduce((total, animal) => {
  return total += animal.population;
}, 0);

populationTotal.push(reducePopulation);
