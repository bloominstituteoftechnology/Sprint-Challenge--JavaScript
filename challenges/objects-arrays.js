...
*/
let displayNames = [];
let newDisplayNames = zooAnimals.forEach((name) => {
...
let lowCaseAnimalNames = zooAnimals.map ((lowers) => {
  return lowers.animal_name.toLowerCase();
});
...
*/
let lowPopulationAnimals = zooAnimals.filter((isLow) => {
  return isLow.population < 5;
});
