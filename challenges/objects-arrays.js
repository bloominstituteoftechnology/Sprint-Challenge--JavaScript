...
const uni = [];
for (let i = 0; i < graduates.length; i++) {
  const checkUni = graduates[i].university.indexOf("Uni");
  if (checkUni >= 0) {
...
*/
const populationTotal = zooAnimals.reduce((populationTotal, currentValue) => {
  return (populationTotal += currentValue.population);
}, 0);
