...
const universities = graduates.map((val)  =>  {
...
const contactInfo = graduates.map((val) =>  {
    return `${val.first_name} ${val.email}`;
});
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = graduates.filter((val)  =>  {
    return val.university.includes("Uni");
});
console.log(uni);
...
const lowerCase = zooAnimals.map((val)  =>  {
    return val.animal_name.toLowerCase();
});
