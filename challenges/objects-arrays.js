...
let universities = [];
universities = graduates.map(elem => elem.university);
universities.sort();
...
const contactInfo = [];
graduates.forEach(elem => contactInfo.push(`${elem.first_name} ${elem.email}`));
...
lowCaseAnimalNames = zooAnimals.map(elem => elem.animal_name.toLowerCase());
...
zooAnimals.filter(elem => {
    if (elem.population < 5) { lowPopulationAnimals.push(elem) }
})
...
*/
let populationTotal = zooAnimals.reduce((accumulator, incrementer) => {
    return accumulator + incrementer.population;
}, 0);
