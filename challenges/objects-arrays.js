...
const contactInfo = [];
for(var i = 0; i < graduates.length; i++){
  var info = graduates["first_name"] + graduates["email"]
...
const uni = [];
for(var i = 0; i < graduates.length; i++){
  let spl = graduates[i]["university"].split("",3).join('')
  console.log(spl);
  if(spl === "Uni"){
...
let populationTotal = [];
populationTotal = zooAnimals.reduce((acc,e)=>{
