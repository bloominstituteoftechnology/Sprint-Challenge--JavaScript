...
function Dinosaur(atr) {
  this.name = atr.name;
  this.diet = atr.diet;
  this.weight = atr.weight;
  this.length = atr.length;
  this.period = atr.period;
}

Dinosaur.prototype.roar = function() {
  return `${this.name} says RAAAAAAAAAWR`;
};
...
var contactInfo = [];
for (var i = 0; i < graduates.length; ++i)
  contactInfo.push(graduates[i]["first_name"], ["email"]);
...
lowerCase = [];
lowerCase = zooAnimals.map(i => i.animal_name.toLowerCase());
...
largerPopulation = zooAnimals.filter(i => i.population < 5);
