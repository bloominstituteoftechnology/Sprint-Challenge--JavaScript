...
const tyrannosaurus = {
  ...
  roar: function () {
    const pleaseDontEatMe = "RAWERSRARARWERSARARARRRR!";
    return pleaseDontEatMe;
  }
};
...
for (let i = 0; i < graduates.length; i++) {
  let colleges = {};
  colleges.university = graduates[i].university;
...
zooAnimals.forEach(function (animals) {
  let animal = `${animals.animal_name} ${animals.scientific_name}`;
...
let lowerCase = zooAnimals.map(function (names) {
...
*/
let largerPopulation = zooAnimals.filter(function (colony) {
  return colony.population <= 5;
});
...
*/
let populationTotal = zooAnimals.reduce(function (sum, pack) {
  return sum + pack.population;
}, 0);
