...
let universities = []
// let universities = graduates.map( (student) => student.university ).sort();
for ( student of graduates) {
...
const temp = [];
for ( student of graduates ){
  student.university.includes('Uni') ? temp.push(student) : null ;
}
for ( student of temp ) {
...
let populationTotal = zooAnimals.reduce( ((counter, animal) => counter += animal.population), 0 );
