...
for (i = 0; i < graduates.length; i++){
  if (graduates[i].university === 'uni'){
    uni.push(JSON.stringify(graduates[i]))
  }
}
console.log(uni);
...
*/
const animalNames = [];
let allAnimalNames = zooAnimals.forEach(zooAnimals => {
...
const lowerCase = [];
let smallNames = zooAnimals.map(zooAnimals => {
...
*/
const largerPopulation = zooAnimals.filter(zooAnimals =>{ return zooAnimals.population < 5
})
