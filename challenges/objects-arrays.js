...
const tyran = {
  ...
  called: "RAWERSRARARWERSARARARRRR!"
}
...
const universities = [];
graduates.map(object => universities.push(object.university))
...
const contactInfo = [];
graduates.forEach(object => contactInfo.push(`graduates:${object.first_name} ${object.email}`));
...
const unisWithUni = [];
graduates.filter(universities => graduates.push(("Uni")));
...
*/
const displayNames = [];
zooAnimals.forEach(n => displayNames.push(`animal_name: ${n.animal_name}: 
...
zooAnimals.map(n => lowCaseAnimalNames.push(n.animal_name.toLowerCase()));
...
const lowPopulationAnimals = zooAnimals.filter(n => n.population < 5);
...
zooAnimals.reduce(function (totalAnimals, zooAnimals) {
  return totalAnimals +
    zooAnimals.population
}, 0);
