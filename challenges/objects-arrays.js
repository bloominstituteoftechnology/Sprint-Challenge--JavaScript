...
*/
class Dino {
  constructor(dinoInfo){
    this.name = dinoInfo.name;
    this.diet = dinoInfo.diet;
    this.weight = dinoInfo.weight;
    this.length = dinoInfo.length;
    this.period = dinoInfo.period;
  }
  roar(){
...
const uni = graduates.map(x => x.university).filter(y => /Uni/.test(y));
console.log(uni);
