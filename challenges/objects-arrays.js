...
for (let i = 0; i < graduates.length; i++) {
  uniList.push(graduates[i].university);
}
...
for (let i = 0; i < uniList.length; i++) {
  let uniWords = uniList[i].split(" ");
  for (word in uniWords) {
    if (uniWords[word].includes("Uni")) {
      uni.push(uniList[i]);
    }
  }
}
...
for (animal in zooAnimals) {
  lowerCase.push(zooAnimals[animal].animal_name.toLowerCase())
}
...
// populate new array with animal populations
for (animal in zooAnimals) populationTotal.push(zooAnimals[animal].population);

// use .reduce()
console.log(populationTotal.reduce((count, total) => count + total));