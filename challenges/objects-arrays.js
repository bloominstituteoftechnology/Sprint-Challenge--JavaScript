...
const universities = [];
for (let i = 0; i < graduates.universities; i++) {
  peopleGraduating.push(graduates[i].universities);
}
console.log(universities);

const contactInfo = [];
for (let i = 0; i < graduates.contactInfo; i++) {
  peopleGraduating.push(graduates[i].contact_Info);
}
...
let lowerCase = [];
animal_name = animalName.lowerCase;
console.log(lowerCase);

// The zoos need to know their total animal population across the United States.  Add up all the population numbers from all the zoos using the .reduce() method.
let populationTotal = [];
.reduce(animal.numbers(populationTotal));
