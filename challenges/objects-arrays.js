...
let tyrannosaurus = {name: "tyrannosaurus", diet: "carnivorous", weight: "7000kg", length: "12m", period: "Late Cretaceious"};
...
function getUniversities(input, field) {
  const universities = [];
...
console.log(getUniversities(graduates, "university"));
...
function getContactInfo(input, field1, field2) {
  const contactInfo = [];
  for (i = 0; i < input.length; i++) {
      contactInfo.push(input[i][field1] + " " + input[i][field2])
  }
  return contactInfo
};
console.log(getContactInfo(graduates, "first_name", "email"));


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
function findUniTotal(input, field) {
  const uni = []
  for (i = 0; i < input.length; i++) {
      if (input[i][field].includes("Uni") === true) {
          uni.push(input[i][field])
      }
  }
  return uni
};

console.log(findUniTotal(graduates, "university"));
...
const populationTotal = zooAnimals.reduce(function(accumulator, currentTotal) {
  return accumulator += currentTotal["population"]
}, 0);
