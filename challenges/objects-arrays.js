...
const contactInfo = [];
for (i=0;i<graduates.length;i++) {
  const studentInfo = `Name: ${graduates[i].first_name}, E-mail:${graduates[i].email} `
...
const lowerCase = [];
zooAnimals.forEach(convertToLowerCase);
...
function arrayPop (item) {
    usPopArray.push(item.population);
};
zooAnimals.forEach(arrayPop);
const populationTotal=usPopArray.reduce(reducer);
