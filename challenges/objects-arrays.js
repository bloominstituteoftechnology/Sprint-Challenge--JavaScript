...
// const universities = graduates.map(grads => grads.university).sort();
...
// const contactInfo = graduates.map(grads => {return {first_name: grads.first_name, email: grads.email}});
// console.log(contactInfo);
const contactInfo = [];
for(let i = 0; i < graduates.length; i++) {
  const result = {first_name: graduates[i].first_name, email: graduates[i].email};
...
// const uni = graduates.map(grads => grads.university).filter(uni => uni.includes('Uni'));
// console.log(uni);
const uni = [];
for(let i = 0; i < graduates.length; i++) {
...
const populationTotal = zooAnimals.map(animal => animal.population).reduce((total, pop) => total + pop, 0);
