...
*/
class Dinosaurs {
  constructor(dinoStats) {
    this.name = dinoStats.name;
    this.diet = dinoStats.diet;
    this.weight = dinoStats.weight;
    this.length = dinoStats.length;
    this.period = dinoStats.period;
  }
  roar() {
...
for (let i = 0; i < graduates.length; i++) {
  if (!(contactInfo.includes(graduates[i].first_name))) {
...
const lowerCase = (x)=>{
  ...
  const lowerCaseAnimals = zooAnimals.map(lowerCase).sort();
lowerCaseAnimals;

// The zoos need to know their total animal population across the United States.  Add up all the population numbers from all the zoos using the .reduce() method.
let allAniPop =[];
newArr = (x) =>{
    allAniPop.push(x.population);
}
zooAnimals.forEach(newArr);

const totaler = (accu, currValue)=>{
    return accu + currValue;
}
let populations = allAniPop.reduce(totaler,0);
populations;