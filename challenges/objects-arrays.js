...
// let newUni = graduates.email(function(){
//   universities.sort();
// });


const universities = [];
let newUni = graduates.sort(function(i){
...
// for (let i = 0; i < graduates.length; i++){
  if (graduates[i].university.includes('uni')){
    uniNew.push(graduates[i].universities);
  }
// }
...
*/
const animalNames = [];
let newNames = zooAnimals.forEach(function(i){
...
const lowerCase = zooAnimals.map((nameLow) => {
  return nameLow.animal_name.toLowerCase();
}
);
...
*/
const largerPopulation = zooAnimals.filter((newPop) => {
  return newPop.population < 5;
});
