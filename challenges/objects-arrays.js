...
const sortedUnies = universities.sort();
console.log(sortedUnies);
...
for (let i in graduates){
  let contact = graduates[i].first_name.concat(' ' + graduates[i].email);
...
populationTotal = zooAnimals.map(x => x.population).reduce((total, amount) => {
  return total + amount;
})
