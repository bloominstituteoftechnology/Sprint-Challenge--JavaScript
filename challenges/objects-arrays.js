...
const lowerCase = [];
lowerCase.push(zooAnimals.map(zoo => zoo.animal_name.toLowerCase()));
...
largerPopulation.push(zooAnimals.filter(item => item.population < 5));
...
let result = zooAnimals.reduce((total, animals) =>
  total + animals.population, 0);
  populationTotal.push(result)
