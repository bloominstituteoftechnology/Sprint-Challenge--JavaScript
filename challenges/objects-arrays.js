...
// graduates.map(item=>item.university).sort();
...
// graduates.map(item=>`${item.first_name} ${item.email}`);
...
// graduates.filter(item=>item.university.includes("Uni"));
console.log(uni);
...
*/
const animalNames = [];
zooAnimals.forEach(item=>animalNames.push(`Name: ${item.animal_name}, ${item.scientific_name}`))
...
*/
const populationTotal = zooAnimals.map(item=>item.population).reduce((a, b)=> a+b);
