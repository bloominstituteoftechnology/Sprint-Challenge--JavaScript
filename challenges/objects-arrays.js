...
let lowerCase = [];
lowerCase = zooAnimals.map(animal => {return animal['animal_name'].toLowerCase()});
...
const pops = zooAnimals.map(animal => {return animal['population']});
populationTotal = pops.reduce((total, amount) => {return total + amount});
