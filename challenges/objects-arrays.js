...
displayNames.push(`${i.animal_name} ${i.scientific_name}`);
});
...
zooAnimals.map((aniLow) => {
lowCaseAnimalNames.push(aniLow["animal_name"].toLowerCase());
});
...
*/
const lowPopulationAnimals = zooAnimals.filter(function (_pop) {
  return _pop.population <= 5;
});
...
*/
const populationTotal = zooAnimals.reduce((reducerFunc, item) => {
return reducerFunc += item.population;
},0);
