...
zooAnimals.map((info) => {
  lowCaseAnimalNames.push(info.animal_name.toLowerCase())
})
...
zooAnimals.filter((info) => {
  if (info.population < 5) {
    lowPopulationAnimals.push(info)
  }
})
...
*/
const populationTotal = zooAnimals.reduce((accum, animal) => accum + animal.population, 0);
