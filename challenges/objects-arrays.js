...
const unisWithUni = graduates.filter(x => {
  return x.university.includes('Uni')
})
...
*/
const displayNames = [];
zooAnimals.forEach(animName => {
  displayNames.push(`Name:${animName.animal_name}, Scientific:${animName.scientific_name}.`)})
...
const lowCaseAnimalNames = zooAnimals.map((lowcNames)=>{
  return lowcNames.animal_name.toLowerCase();
})
...
*/
const populationTotal = zooAnimals.reduce((accumulator, pops) =>{
