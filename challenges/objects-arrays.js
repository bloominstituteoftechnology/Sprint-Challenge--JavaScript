...
for (let graduate of graduates) {
    if (graduate.first_name && graduate.email) contactInfo.push(`${graduate.first_name} ${graduate.email}`);
}
...
for (let graduate of graduates) {
    if (graduate.university && graduate.university.toUpperCase().includes("UNI")) uni.push(graduate.university);
}
...
for (let animal of zooAnimals) {
    if (animal.animal_name) lowerCase.push(animal.animal_name.toLowerCase());
}
