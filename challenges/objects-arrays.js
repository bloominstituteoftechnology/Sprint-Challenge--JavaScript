...
for(let i = 0; i < universities.length; i++) {
  //console.log(universities[i].match(/uni/g))
  if(universities[i].match(/[uni]/gi, [i])) {
    console.log(universities.indexOf(universities[i]))
...
// line 126 contradicts line 129... are we looking for largest or smallest?

zooAnimals.filter(animals => {
  if(animals.population > 5) {
    largerPopulation.push(animals.population)
  }
...
let pop = [];
zooAnimals.map(p => {
  pop.push(p.population);
  return pop;
})
populationTotal = pop.reduce((acc, curr) => {
  return acc + curr;
}, 0);
console.log(populationTotal)

// zooAnimals.reduce((curr, nex) => {
//   curr.population + nex.population;
// });
