...
const contactInfo = [];
graduates.forEach(E => {
  return contactInfo.push(`${E.first_name}, ${E.email}`);
});
...
graduates.forEach(e => {
  if (e.university.includes("Uni")) {
    return unisWithUni.push(e.university);
  }
});
...
*/
const displayNames = [];
zooAnimals.forEach(E => displayNames.push(E.scientific_name, E.animal_name));
