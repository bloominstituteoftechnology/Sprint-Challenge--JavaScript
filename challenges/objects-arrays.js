...
class Dinosaur {
  constructor (opti) {
    this.name = opti.name;
    this.diet = opti.diet;
    this.weight = opti.weight;
    this.length = opti.length;
    this.period = opti.period;
  }
  roar() {
...
//const universities = [];
let universities = [];
  universities = graduates.map((curVal, i) => {
  return graduates[i].university;
});
universities.sort();
...
let contactInfo = [];
  contactInfo = graduates.map((curVal, i) => {
  return graduates[i].first_name + " " + graduates[i].email;
});
...
*/
const animalNames = [];
zooAnimals.forEach(ani => animalNames.push(`Name: ${ani.animal_name}, Scientific: ${ani.scientific_name}.`));
...
let lowerCase = [];
lowerCase = zooAnimals.map(ani => ani.animal_name.toLowerCase())
...
largerPopulation = zooAnimals.filter(ani => ani.population < 5);
