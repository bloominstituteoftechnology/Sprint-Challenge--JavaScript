...
//   function bongo(item) {
//     uni.push(item.university.includes('Uni'))
//   }

//   graduates.forEach(bongo)
...
function names(item) {
  animalNames.push(item.animalNames, item.scientific_name)
}
...
function regName(item) {
  lowerCase.push(item.animal_name.toLowerCase())
}

zooAnimals.map(regName)
...
lowerPopulation = zooAnimals.filter( endangered => endangered.population < 5);
...
var populationTotal = zooAnimals.reduce(function(sum, zooAnimals) {
