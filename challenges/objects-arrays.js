...
universities.sort() // didn't pass any arguments because each Uni is capital letter and wont have any conflict with order
...
for (let i = 0; i < graduates.length; i++) {
  graduates[i].university.toLowerCase().includes('uni') ? uni.push(graduates[i].university) : '';
}
...
*/
const populationTotal = zooAnimals.reduce((acc, total) => acc + total.population, 0);
