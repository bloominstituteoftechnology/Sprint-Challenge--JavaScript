...
console.log(`The diet of a velociraptor was ${velociraptor.diet}`);

// How long was a stegosaurus?
console.log(`A stegosaurus was around ${stegosaurus.length} long`);
...
/* Request 1: Create a new array called universities that contains all the univeristies in the graduates array. */
  ...
  for (let index = 0; index < graduates.length; index++) {
    univeristies.push(graduates[index]["university"]);
  }
/*Once you have the new array created, sort the universities alphabetically and log the result. */
const sortedUniversities = [];
for (let i = 0; i < univeristies.length; i++) {
  let counter = 0;
  for (let j = 0; j < sortedUniversities.length; j++) {
      if (univeristies[i] > sortedUniversities[j]) {
          counter++;
      }
  }
  sortedUniversities.splice(counter, 0, univeristies[i])
}
...
const uni = [];
for (let index = 0; index < sortedUniversities.length; index++) {
  if (sortedUniversities[index].includes("Uni")) {
    uni.push(sortedUniversities[index])
  }
}
console.log(uni);
