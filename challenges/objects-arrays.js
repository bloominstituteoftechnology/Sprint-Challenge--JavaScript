...
const uni = [];
for(let i = 0;i < graduates.length; i++){
  uni.push(graduates[i].university.includes('Uni').length)
}
console.log(uni);
...
*/
const animalNames = zooAnimals.forEach(x => {
  console.log(`${x.animal_name}, ${x.scientific_name}`)
});
...
*/
const population = zooAnimals.map(x => x.population)
const populationTotal = population.reduce((a,b) => a + b);
