...
const lowCaseAnimalNames = displayNames.map(lowercase);
...
const popinfo = pop => pop.population < 5;
lowPopulationAnimals = zooAnimals.filter(popinfo);
...
*/
let populationTotal = [];
const totalPop = (acc, pop) => acc + pop;
zooAnimals.forEach(function(item) {
  populationTotal.push(item.population)
});
populationTotal = populationTotal.reduce(totalPop);
