...
const contactInfo = [];
for (let i = 0; i < graduates.length; i++) {
  return contactInfor.push(`${graduates[i].first_name} ${graduates[i].email}`)
}
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = []; //come back to this later
...
*/
const animalNames = []
zooAnimals.forEach(name => animalNames.push(`${name.scientific_name} ${name.animal_name}`))
...
const lowerCase = [];
zooAnimals.map(lowercaseNames => lowerCase.push(`${lowercaseNames.animal_name.toLowerCase()}`))
...
zooAnimals.filter(pop => {
  if(pop.population < 5) {
    largerPopulation.push(`${pop.population} ${pop.animal_name}`)
  }
})
