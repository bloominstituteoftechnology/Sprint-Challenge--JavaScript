...
// Given an array of college graduates.  Complete the following requests 
// without using any array methods like .forEach(), .map(), .reduce(), .filter()
...
/* Request 1: Create a new array called universities that contains all 
   the univeristies in the graduates array.
 
Once you have the new array created, sort the universities alphabetically 
and log the result. */
const universities = [];
...
const contactInfo = [];
for (let i = 0; i < graduates.length; i++) {
  let studentInfo = `${graduates[i].first_name} ${graduates[i].email}`;
