...
for (let i=0;i<graduates.length;i++){
  if (graduates[i].university.includes("Uni")){
    ...
    do {
      if (uni[iterator] === graduates[i]){
        isDuplicate = true;
      } 
      iterator++;
    } while ((uni[iterator] != graduates[i]) && (iterator <= i));
...
let lowerCase = [];
lowerCase = zooAnimals.map(function (currentAnimal) {
  return currentAnimal["animal_name"].toLowerCase();
});
console.log(lowerCase); 

// The zoos need to know their total animal population across the United States.  Add up all the population numbers from all the zoos using the .reduce() method.
let populationTotal = [];
populationTotal = zooAnimals.reduce(function(accumulator,currentAnimal){
