...
const emmazingsaurus = {
	"name": "emmazingsaurus",
	"diet": "whatever she finds",
	"weight": "unknown",
	"length": "human height",
	"period": "Late 90's"
};
...
const uni = [];
	for (let i = 0; i < graduates.length; i++) {
  	let value = graduates[i].university;
  		if (value.includes("Uni")) {
...
zooAnimals.map(function(letters){
  lowerCase.push(letters.animal_name.toLowerCase());
});
