...
const contactInfo = [];
contactInfo.push(graduates.filter(graduate => contactInfo.push(`${graduate.first_name}:${graduate.email}`)))
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = [];
uni.push(graduates.filter(graduate => graduate.university.includes('Uni')));
...
const lowerCase = [];
lowerCase.push(zooAnimals.map(zooAnimalz=> zooAnimalz.animal_name.toLowerCase()))
console.log(lowerCase); 

// The zoos need to know their total animal population across the United States.  Add up all the population numbers from all the zoos using the .reduce() method.
const populationTotal = [];
let reducer = (sum,order) => sum + order;
populationTotal.push(zooAnimals.map(zooAnimalz => zooAnimalz.population).reduce(reducer,0))
