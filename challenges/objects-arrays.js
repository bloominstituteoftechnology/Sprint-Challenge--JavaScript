...
zooAnimals.forEach(animal => animalNames.push(`AnimalName: ${animal.animal_name}, ScientificName: ${animal.scientific_name}`))
...
// const largerPopulation = zooAnimals.filter(lowpopcount => lowpopcount.population < 5)

const largerPopulation = zooAnimals.filter(function (lowpopcount) {
 return lowpopcount.population < 5 })
...
*/
const populationTotal = zooAnimals.reduce((accu, current) => {
  return accu + current.population;
},0);
