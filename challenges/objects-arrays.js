...
for (let i = 0; i < graduates.length; i++) {
 ...
 // with array method
universities = graduates.map(graduate => graduate.university).sort();
...
for (let i = 0; i < graduates.length; i++) {
    if (graduates[i].university.includes("Uni")) {
      uni2.push(graduates[i].university);
    } 
  
}
// with array methods
uni = graduates
  .filter(graduate => graduate.university.includes("Uni"))
...
lowerCase = zooAnimals.map(animal => animal.toLowerCase())
