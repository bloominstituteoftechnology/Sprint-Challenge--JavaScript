...
const universities = graduates.map((x) => {
  return x.university;
});
universities.sort();
...
const contactInfo = graduates.map((c) => {
  return `${x.first_name} ${x.email}`;
});
...
zooAnimals.forEach((x) =>
  displayNames.push(`Name: ${animal_name}, Science: ${x.scientific_name}`)
);
