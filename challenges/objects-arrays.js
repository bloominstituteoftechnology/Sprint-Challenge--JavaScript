...
for (let each of graduates){
  universities.push(each.university);
  universities.sort();
}
...
for (let each of graduates){
  contactInfo.push(`${each.first_name} ${each.email}`)
}
...
for(let each of universities){
  if (each.includes("Uni")){
    uni.push(each);
  }
}
...
let names = [];
for (let each of zooAnimals){
  names.push(each.animal_name);
}

let lowerCase = names.map(function(x) {
...
for (let each of zooAnimals){
  numArray.push(each.population);
  let populationTotal = numArray.reduce(function( acc, c) {
    return acc + c });
}
