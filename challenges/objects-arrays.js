...
const contactInfo = [];
for(let info = 0; info < graduates.length; info++) {
  contactInfo.push(`${graduates[info].first_name} ${graduates[info].email}`);
}
...
let populationTotal = zooAnimals.reduce((total, current) => (total += current.population), 0);
