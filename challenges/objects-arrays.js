...
let universities = [];
const universitiyArray = graduates => {
...
const contactInfo = [];
const contactArray = graduates => {
  for (let i = 0; i < graduates.length; i++) {
    contactInfo.push(`${graduates[i].first_name} ${graduates[i].email}`);
  }
};

contactArray(graduates);
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */

const uni = [];
const containUni = graduates => {
  for (let i = 0; i < graduates.length; i++) {
...
*/
const animalNames = [];
zooAnimals.forEach(n => {
  animalNames.push(`Name: ${n.animal_name}, Scienctific: ${n.scientific_name}`);
});
...
const lowerCase = zooAnimals.map(n => {
  return `Name: ${n.animal_name.toLowerCase()}`;
});
...
const largerPopulation = zooAnimals.filter(n => {
...
*/
const populationTotal = zooAnimals.reduce((popTotal, indiPop) => {
  return popTotal + indiPop.population;
}, 0);
