...
for (let i of graduates) {
  universities.sort().push(i.university);
}
...
const contactInfo = [];
//graduates.map(i => contactInfo.push(`${i.first_name} ${i.email}`));
for (let i of graduates) {
...
let populationTotal = [];
const reduced = zooAnimals.reduce((result, i) => {
  return result += i.population;
}, 0);
