...
const contactFunction = (arr, i) => {
  for (var i = 0; i < graduates.length; i++) {
    contactInfo.push(graduates[i].first_name + " " + graduates[i].email);
  }
};
contactFunction(graduates);
console.log(contactInfo);
...
lowerCase = zooAnimals.map(mappedCB);
console.log(lowerCase);
...
const reduceCB = (accumulator, currentValue, i) => {
  currentValue = zooAnimals[i].population;
  return accumulator + currentValue;
};
populationTotal = zooAnimals.reduce(reduceCB, 0);
console.log(populationTotal);
