...
console.log(uni);
console.log("Total number of universities having Uni: " + uni.length);
...
const animalNamesNew = [];
zooAnimals.forEach((items) => {
    animalNamesNew.push("Name:"+ items.animal_name + ", " + "Scientific:"+ items.scientific_name + ".");
    return animalNamesNew;
})

console.log(animalNamesNew);


const lowerCaseNew = zooAnimals.map((name)=>{
    return name.animal_name.toLowerCase();
})
console.log(lowerCaseNew);


const largerPopulationNew = zooAnimals.filter((animal)=>{
    return animal.population<5;
})
console.log(largerPopulationNew);


const populationTotalNew=
    zooAnimals.reduce((sum,animal)=>{
        return sum+animal.population;
    },0)
console.log(populationTotalNew);
