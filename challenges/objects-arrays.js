...
let universityNames = graduates.forEach(function(i){
  graduates.push(i.universities);
...
let nameEmail = graduates.forEach(function(i) {
...
const uni = [];
let abbreviation = grad.filter((uni) => {
  abbreviation.push(uni);
...
let lowerCase = [];
let allLowercase = zooAnimals.map((lowerCase) => {
    return zooAnimals.last_name.toLowerCase();

})
console.log(lowerCase);

// The zoos need to know their total animal population across the United States.  Add up all the population numbers from all the zoos using the .reduce() method.
let populationTotal = zooAnimals.reduce((population, population_total) =>{
return population + population_total;
}, 0);
