...
const universities = [];
graduates.forEach(function(uniz){
  universities.push(`${uniz.university}`);
});
...
graduates.forEach(function(contactt){
  contactInfo.push(`${contactt.first_name},${contactt.email}`);
});
...
graduates.map(uni=>{
  uni.university.includes("Uni") ? unisWithUni.push(uni) : null;
});
...
zooAnimals.filter(population => {
  population.population < 5 ? lowPopulationAnimals.push(population):null;
});
