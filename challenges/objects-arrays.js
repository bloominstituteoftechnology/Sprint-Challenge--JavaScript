...
graduates.filter(school => {if (school.university === 'Uni') {uni.push(school)}});
...
zooAnimals.map(element => lowerCase.push(element.animal_name.toLowerCase()));
...
zooAnimals.filter(element => {if (element.population < 5 ) {lowerPopulation.push(element)}});
...
populationTotal = zooAnimals.reduce((total, pop) => total + pop.population);
