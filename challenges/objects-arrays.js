...
const universities = [];
for(let graduateIndex = 0; graduateIndex < graduates.length; graduateIndex++){
  const indexedGraduate = graduates[graduateIndex];
  const testUniversity = indexedGraduate.university;
  if(universities.includes(testUniversity)){ continue;}
...
for(let graduateIndex = 0; graduateIndex < graduates.length; graduateIndex++){
  const indexedGraduate = graduates[graduateIndex];
  contactInfo.push(`${indexedGraduate.first_name} ${indexedGraduate.email}`);
}
...
const uni = [];
for(let uniIndex = 0; uniIndex < universities.length; uniIndex++){
  const indexedUni = universities[uniIndex];
  if(!indexedUni.includes("Uni")){ continue;}
  uni.push(indexedUni);
}
console.log(uni);
...
const populationTotal = zooAnimals.reduce(
  (currentValue, animal) => { return currentValue + animal.population},
  0
);
console.log(populationTotal);

// We spared no expense.
zooAnimals.push(tyrannosaurus, stegosaurus, velociraptor);
