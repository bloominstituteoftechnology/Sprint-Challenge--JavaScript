...
for(let graduate in graduates) {
  if(graduates[graduate].university.indexOf("Uni") != -1) {
    uni.push(graduates[graduate].university);
  }
}
 console.log(uni);
...
*/
zooAnimals.forEach(bothNames => console.log(`Animal Name: ${bothNames.animal_name} -- Scientific Name: ${bothNames.scientific_name}`));
...
zooAnimals.map(nameList => console.log(nameList.animal_name.toLowerCase()));
...
const lowerPopulation = zooAnimals.filter((lPop) => {return lPop.population < 5;});
...
const populationTotal = zooAnimals.reduce((populationTotal, item) => populationTotal + item.population, 0);
