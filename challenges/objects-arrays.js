...
const universities = graduates.map(gradunivers => gradunivers.university);
console.log(universities);
const sortUni = universities.sort();
console.log(sortUni);
...
const contactInfo = graduates.filter(grads => `${first_name},${email}`);
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = graduates.filter(uni => uni.university === "Uni");
console.log(unisWithUni(graduates, "Uni"));
...
*/ const displayNames = zooAnimals.forEach((name)=>name.animal_name +"," + name.scientific_name)
...
const lowPopulationAnimals = zooAnimals.filter(
  populate => populate.population < 5
);
...
I think i already bput them in array. Tutoring helped me understand how.
