...
let nameInfo = [];
for(let i=0; i<graduates.length; i++) {
nameInfo.push(graduates[i]["first_name"]);
let emailInfo = [];
emailInfo.push(graduates[i]["email"]);
...
// The zoos need a list of all their animal's names converted to lower case.  Create a new array named lowerCase and map over each name to convert them all to lower case.  Log the result.
let lowerCase = zooAnimals.map(function(critters) {
  return critters.animal_name.toLowerCase();
});
console.log(lowerCase);

// The zoos need to know their total animal population across the United States.  Add up all the population numbers from all the zoos using the .reduce() method.
const populationTotal =zooAnimals.reduce((theReducer, items) =>{
  return theReducer += items.population;
}, 0);
