...
*/
const animalNames = [];
zooAnimals.forEach(({ animal_name, scientific_name }) => animalNames.push(`Name: ${animal_name} Scientific: ${scientific_name}`));
...
const lowerCase = [];
zooAnimals.map(({ animal_name }) => lowerCase.push(animal_name.toLowerCase()));
...
largerPopulation.push(zooAnimals.filter(({ population }) => population < 5));
...
zooAnimals.reduce((acc, curr) => (populationTotal = acc + curr.population), 0);
