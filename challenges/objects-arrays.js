...
const uni = [];
for (i=0 ; i < graduates.length ; i++) {
  let collegeName = graduates[i].university.split('');
  for (j=0 ; j < collegeName.length ; j++) {
    if ((collegeName[j] === 'U' || collegeName[j] === 'u') && (collegeName[j+1] === 'N' || collegeName[j+1] === 'n') && (collegeName[j+2] === 'I' || collegeName[j+2] === 'i')) {
      uni.push(collegeName.join(''));
      j = collegeName.length;
    };
  };
};
console.log(uni);
...
let populationTotal = zooAnimals.reduce((r, animal) => {
  return r += animal.population;
}, 0);
