...
//make sure to change searchVal if testing with direct console 
for(i=0; i< graduates.length; i++){
...
zooAnimals.forEach(function(zooAnimals){
  displayNames.push(`${zooAnimals.scientific_name} ${zooAnimals.animal_name}`)
})
...
let lowCaseAnimalNames = zooAnimals.map((eachAnimal,index,zooAnimals)=>{
  return eachAnimal.animal_name.toLowerCase();
});
...
*/
let totalPopulation = zooAnimals.reduce((population, eachAnimal,indexedDB,zooAnimals)=>{
  // return population = population + eachAnimal.population;
     return population +=eachAnimal.population;
 },0)//initial value issues
