...
for(let i= 0; graduates.length > i; i++) {
...
zooAnimals.map(function(value){
    return lowerCase.push(value.animal_name.toLowerCase());
});
...
let populationTotal = [];
zooAnimals.reduce((theReducer, item) => {
  return populationTotal = theReducer + item.population;
}, 0);
