...
function Dinosaur (dinosaur_attributes) {
  this.name = dinosaur_attributes.name;
  this.diet = dinosaur_attributes.diet;
  this.weight= dinosaur_attributes.weight;
  this.length = dinosaur_attributes.length;
  this.period = dinosaur_attributes.period;
}
...
uni that contains them all. Log the result. */
const uni = [];
...
// The zoos need a list of all their animal's names converted to lower case.  Create a new array named lowerCase and
...
makeLower(zooAnimals);

console.log(lowerCase);


// The zoos need to know their total animal population across the United States.  Add up all the population numbers
// from all the zoos using the .reduce() method.
const populationTotal = [];
function getTotalPop(arrObj) {
  return arrObj.reduce((acc, curr) => acc + curr.population, 0);
}
