...
const map = (elements, cb) => {
  ...
  //   let cbArr = [];
  //   let valAintoCB = cb(element);
  //   cbArr.push(valAintoCB);
  // })
  const passArray = [];
...
// Create a CuboidMaker constructor function that accepts properties for length, width, and height <<<DONE
...
function CuboidMaker1(options) {
  this.length = options.length;
  this.width = options.width;
  this.height = options.height;
}
CuboidMaker1.prototype.volume = function () {
  let cuboidVol = this.length * this.width * this.height;
  return cuboidVol;
}
